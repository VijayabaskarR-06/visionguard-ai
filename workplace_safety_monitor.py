...
class AssocConfig:
    ...
    head_iou_gate: float = 0.10         
    torso_iou_gate: float = 0.15        
    ppe_inside_person_frac: float = 0.35 
...
class Monitor:
    ...
    def process_frame(self, frame: np.ndarray) -> np.ndarray:
        ...
        track_items = sorted(tracks.items(), key=lambda kv: kv[0])  
...
def build_argparser() -> argparse.Namespace:
    ...
    p.add_argument('--no-show', action='store_true', help='Disable GUI window (imshow)')
